...
/**
 * Script to create a superadmin account in Firebase
 * Run this script using: node make-superadmin.js email@example.com
 */
...
if (!email) {
  console.error('Please provide an email address: node make-superadmin.js email@example.com');
  process.exit(1);
}
...
async function makeSuperAdmin() {
  try {
    ...
    console.log(`Successfully set superadmin privileges for ${email} (${user.uid})`);
    ...
    if (users) {
      ...
      console.log(`Updated user record in database with superadmin role`);
    } else {
