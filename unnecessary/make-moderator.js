...
/**
 * Script to create a moderator account in Firebase
 * Run this script using: node make-moderator.js email@example.com
 */
...
if (!email) {
  console.error('Please provide an email address: node make-moderator.js email@example.com');
  process.exit(1);
}
...
async function makeModerator() {
  try {
    ...
    console.log(`Successfully set moderator privileges for ${email} (${user.uid})`);
    ...
    if (users) {
      ...
      console.log(`Updated user record in database with moderator role`);
    } else {
